...
#define _ASSERT(result, condition, ...) if(!(condition)) (result) = false; ASSERT(condition, VOID, __VA_ARGS__)

void test_buf_insert_pseudo(bool* out_result)
{
	BUFFER buffer = buf_create(sizeof(u32), 5, 0);
	...
	buf_push_u32(&buffer, 3506);
	buf_push_u32(&buffer, 1806);
	buf_push_u32(&buffer, 1880);
	buf_push_u32(&buffer, 149);
	_ASSERT(*(out_result), buf_get_capacity(&buffer) == 5, "Either buf_insert_pseudo or buf_get_capacity isn't working correctly");
	buf_push_u32(&buffer, 13);
	_ASSERT(*(out_result), buf_get_capacity(&buffer) == 10, "Either buf_insert_pseudo or buf_get_capacity isn't working correctly");
	buf_push_u32(&buffer, 1423);
	buf_push_u32(&buffer, 1253);
	buf_push_u32(&buffer, 1423);
	buf_push_u32(&buffer, 153);
	buf_push_u32(&buffer, 163);
	buf_push_u32(&buffer, 723);
	buf_push_u32(&buffer, 823);
	buf_push_u32(&buffer, 323);
	_ASSERT(*(out_result), buf_get_capacity(&buffer) == 20, "Either buf_insert_pseudo or buf_get_capacity isn't working correctly");
	_ASSERT(*(out_result), buf_get_element_count(&buffer) == SIZEOF_ARRAY(test_values), "buf_get_element_count() isn't working correctly");

	for(u32 i = 0; i < buf_get_element_count(&buffer); i++)
	{
		_ASSERT(*(out_result), buf_get_at_typeof(&buffer, u32, i) == test_values[i], "buf_push isn't working correctly");
	}

	buf_insert_pseudo(&buffer, 2, 4);
	_ASSERT(*(out_result), buf_get_element_count(&buffer) == (SIZEOF_ARRAY(test_values) + 4), "buf_insert_pseudo isn't workrig correctly");

	for(u32 i = 0; i < 2; i++)
	{
		_ASSERT(*(out_result), buf_get_at_typeof(&buffer, u32, i) == test_values[i], "The data before the insert index is corrupted due to buf_insert_pseudo");
	}

	for(u32 i = 6; i < buf_get_element_count(&buffer); i++)
	{
		_ASSERT(*(out_result), buf_get_at_typeof(&buffer, u32, i) == test_values[i - 4], "The data after the (insert index + insert count) is corrupted due to buf_insert_pseudo");
	}

	buf_remove_pseudo(&buffer, 2, 4);

	_ASSERT(*(out_result), buf_get_element_count(&buffer) == SIZEOF_ARRAY(test_values), "buf_remove_pseudo() isn't working correctly");

	for(u32 i = 0; i < 2; i++)
	{
		_ASSERT(*(out_result), buf_get_at_typeof(&buffer, u32, i) == test_values[i], "The data before remove index is corrupted due to buf_remove_pseudo()");
	}

	for(u32 i = 2; i < buf_get_element_count(&buffer); i++)
	{
		_ASSERT(*(out_result), buf_get_at_typeof(&buffer, u32, i) == test_values[i], "The data after (remove index + remove count) is corrupted due to buf_remove_pseudo()");
	}

	u32 prev_value = buf_get_at_typeof(&buffer, u32, SIZEOF_ARRAY(test_values) - 1);
	u32 value = 1234567;
	buf_insert_at(&buffer, SIZEOF_ARRAY(test_values) - 1, &value);
	_ASSERT(*(out_result), buf_get_element_count(&buffer) == (SIZEOF_ARRAY(test_values) + 1), "buf_insert_at isn't working correctly");
	_ASSERT(*(out_result), buf_get_value_at_typeof(&buffer, u32, SIZEOF_ARRAY(test_values) - 1) == value, "buf_insert_at isn't working correctly");
	_ASSERT(*(out_result), buf_get_value_at_typeof(&buffer, u32, SIZEOF_ARRAY(test_values)) == prev_value, "buf_insert_at isn't working correctly");

	value = 54321;
	buf_insert_at(&buffer, SIZEOF_ARRAY(test_values) + 1, &value);
	_ASSERT(*(out_result), buf_get_element_count(&buffer) == (SIZEOF_ARRAY(test_values) + 2), "buf_insert_at isn't working correctly");
	_ASSERT(*(out_result), buf_get_value_at_typeof(&buffer, u32, SIZEOF_ARRAY(test_values) + 1) == value, "buf_insert_at isn't working correctly");

	buf_remove_at(&buffer, 13, NULL);
	_ASSERT(*(out_result), buf_get_value_at_typeof(&buffer, u32, buf_get_element_count(&buffer) - 1) == 54321, "buf_remove_at isn't working correctly");
	buf_remove_at(&buffer, buf_get_element_count(&buffer) - 1, NULL);

	for(u32 i = 0; i < SIZEOF_ARRAY(test_values); i++)
	{
		_ASSERT(*(out_result), buf_get_at_typeof(&buffer, u32, i) == test_values[i], "buf_remove_at isn't working correctly");
	}

	buf_insert_pseudo(&buffer, 4, 9);
	_ASSERT(*(out_result), buf_get_element_count(&buffer) == (SIZEOF_ARRAY(test_values) + 9), "buf_insert_pseudo isn't workrig correctly");
	...
	for(u32 i = 13; i < buf_get_element_count(&buffer); i++)
	{
		_ASSERT(*(out_result), buf_get_at_typeof(&buffer, u32, i) == test_values[i - 9], "The data after the (insert index + insert count) is corrupted due to buf_insert_pseudo");
	}
